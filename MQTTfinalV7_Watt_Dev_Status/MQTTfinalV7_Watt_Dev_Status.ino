  ...
  while (!client.connected()) 
  {
    ...
    if (client.connect(clientId.c_str()))
    {
     ...
     //once connected to MQTT broker, subscribe command if any
      ...
      Serial.println("Subscribed to the topic:    ds/sl/st");
    } else {
...
void sendStatus(){
           ...
           if((digitalRead(ledPin2) == HIGH)){//Fan
...
int minValue = 1024;       // store min value here
   ...
   long now = millis();
   if((now - lastMsgTime) > 1000) //sample for 1 Sec
   {
       lastMsgTime = now;
       //Write code here
